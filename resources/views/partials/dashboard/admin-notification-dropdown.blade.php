        ...
        async init() {
         ...
         async fetchNotifications() {
             try {
                 ...
                 const response = await fetch('{{ url('/api/admin/notifications') }}', {
                     headers: {
         ...
         // Optimistic mark-as-read on hover
         async markAsReadOptimistic(notification) {
             try {
                ...
                notification.is_read = true;
                 ...
                 const token = localStorage.getItem('token');
                 const resp = await fetch(`{{ url('/api/admin/notifications') }}/${notification.id}/read`, {
                     method: 'POST',
                     headers: {
                         'Authorization': `Bearer ${token}`,
                         'Accept': 'application/json'
